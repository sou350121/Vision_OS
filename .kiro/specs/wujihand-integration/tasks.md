...
- [x] 1. Set up testing infrastructure
  - [x] 1.1 Add Python testing dependencies (pytest, hypothesis) to requirements.txt
...
- [x] 6. Implement WebSocket protocol tests
  - [x] 6.1 Write unit tests for hello-status exchange
    - Test that hello message triggers status response
    - Verify status contains required fields
    - _Requirements: 1.3_
  - [x] 6.2 Write property test for hello-status round trip
    - **Property 10: Hello-Status Round Trip**
    - *For any* hello message, a status response SHALL be sent
    - **Validates: Requirements 1.3**
  - [x] 6.3 Write property test for status broadcast
    - **Property 8: Status Broadcast to All Clients**
    - *For any* status change, ALL connected clients SHALL receive update
    - **Validates: Requirements 2.5**

- [x] 7. Checkpoint - Verify all Bridge tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [x] 8. Implement VisionOS extension calculation tests
  - [x] 8.1 Create JavaScript test setup (vitest)
    - Add vitest to package.json devDependencies
    - Configure for ES modules
    - _Requirements: Testing Strategy_
  - [x] 8.2 Write unit tests for finger extension calculation
    - Test `getFingerExtension` with mock landmarks
    - Test all 5 fingers (thumb uses different logic)
    - _Requirements: 3.1_
  - [x] 8.3 Write property test for extension value range
    - **Property 1: Extension Value Range Invariant**
    - *For any* valid landmarks, extension SHALL be in [0, 100]
    - **Validates: Requirements 3.1**

- [x] 9. Implement VisionOS hand selection tests
  - [x] 9.1 Write unit tests for hand selection filtering
    - Test LEFT selection sends only left-hand data
    - Test RIGHT selection sends only right-hand data
    - _Requirements: 8.2, 8.3_
  - [x] 9.2 Write property test for hand selection filtering
