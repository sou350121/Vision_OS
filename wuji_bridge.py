...
class WujiBridge:
    ...
    def _filter_target(self, desired: np.ndarray) -> np.ndarray:
        """Apply smoothing + speed limiting globally (for ALL commands) and update last_target."""
    ...
    def _write_raw(self, target: np.ndarray) -> None:
        ...
        if self.cfg.write_mode == "unchecked":
            # Unchecked write returns immediately and avoids blocking/timeouts during high-rate streaming.
            self.hand.write_joint_target_position_unchecked(arr, timeout)
        else:
            # Blocking write waits for completion (can timeout if called too frequently).
...
def _parse_args() -> Config:
    ...
    p.add_argument("--telemetry-hz", type=float, default=10.0)
    p.add_argument("--smoothing", type=float, default=0.35, help="0..1, higher = less smoothing")
    p.add_argument("--max-speed", type=float, default=0.08, help="Max joint target speed (rad/s). 0 disables limiting.")
    p.add_argument(
        "--unjam-max-speed",
        type=float,
        default=0.12,
    ...
    p.add_argument("--max-curl", type=float, default=0.70, help="Max curl (0=open, 1=full close). Prevents 'perfect fist'.")
    ...
    p.add_argument("--arm-reset-s", type=float, default=20.0, help="Seconds after ARM to force OPEN (reset) before taking tracking commands.")
