"""
Emergency Unjam Script - 使用 LOWER 作為 OPEN 方向
"""
...
def unjam_hand():
    print("=" * 50)
    print("🔧 WujiHand 緊急解卡 (LOWER = OPEN)")
    print("=" * 50)
    ...
    # ⚠️ 這次用 LOWER 作為 OPEN 位置！
    open_pose = np.array(lower, dtype=np.float64)
    print(f"\n  🎯 目標 OPEN 位置 (使用 LOWER): {open_pose[0]}")
    
    # Step 2: 降低電流
    print("\n[2/6] 降低電流限制到 400mA...")
    try:
        hand.write_joint_current_limit(400, 2.0)
        print("✅ 電流已降低")
    except Exception as e:
        print(f"⚠️ 設定電流失敗: {e}")
    
    # Step 3: Disable (鬆力)
    print("\n[3/6] Disable joints (鬆力 5 秒)...")
    try:
    ...
    # Step 6: 順序打開手指 (往 LOWER 方向)
    print("\n[6/6] 順序打開手指 (往 LOWER 方向)...")
    ...
    try:
        ...
        current_pos = np.array(upper, dtype=np.float64)  # 假設卡在 upper (握拳)
